...
import { signUpUser as signUpUserAction } from "../../store/signUpSlice";
...
function Signup() {
  ...
  const [name, setName] = useState(""); // Additional field for signup
  ...
  const handleSubmit = async (e) => {
    ...
    const data = {
      ...
      password: password,
      name: name,
      phoneNumber: phone // Send name along with the email and password
    };

    try {
      setIsLoading(true); // Start loading
      const result = await signUpUser(data); // Call signup function
  ...
  return (
    <div className="px-4 lg:px-0 grid grid-cols-1 md:grid-cols-2 overflow-hidden h-screen bg-main bg-[url('https://images.pexels.com/photos/4502111/pexels-photo-4502111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1')] lg:bg-none">
      ...
      <div className="flex justify-center max-h-screen">
        <form className="border-primary border-solid border-2 w-full lg:w-2/3 rounded-lg h-fit my-auto px-4 lg:px-10 py-5" onSubmit={handleSubmit}>
        ...
        >
          ...
          <div>
            <div className="bg-primary my-2 border-2 border-solid rounded-full px-4 py-1 text-main text-xl w-full">
              <button className="text-main text-lg w-full" type="submit" disabled={isLoading}>
